...
import com.inlaco.crewmgrservice.feature.apikey.presentation.dto.request.CreateApiKeyWithOtpRequest;
...
public class ApiKeyManagementController {
  ...
  public ResponseEntity<TotpSetupResponse> setupApiKeyCreationTotp(@CurrentUser User user) {
    ...
    TotpSetupResponse response =
        totpUseCase.setupTotp(userId, email, TotpSecret.TotpPurpose.API_KEY_CREATION, purposeId);
  ...
  @PreAuthorize("hasRole('ADMIN')")
  @Operation(summary = "Create a new API key with OTP verification (TOTP or Email/SMS)")
  public ResponseEntity<ApiKeyResponse> createApiKeyWithOtp(
      @Valid @RequestBody CreateApiKeyWithOtpRequest request, @CurrentUser User user) {
    ...
    try {
      ...
      if (request.isTotpVerification()) {
        // Create API key with TOTP verification
        CreateApiKeyWithTotpRequest totpRequest = new CreateApiKeyWithTotpRequest();
        totpRequest.setClientName(request.getClientName());
        totpRequest.setDescription(request.getDescription());
        totpRequest.setType(request.getType());
        totpRequest.setTotpCode(request.getTotpCode());
        totpRequest.setPurposeId(request.getTotpPurposeId());

        apiKey = apiKeyService.generateNewWithTotp(totpRequest, userId);

      } else if (request.isNormalOtpVerification()) {
        // Create API key with normal OTP verification
        boolean otpValid =
            otpUseCase.verifyOtp(
                userId,
                request.getOtpCode(),
                Otp.OtpPurpose.API_KEY_CREATION,
                request.getOtpPurposeId());

        if (otpValid) {
          apiKey =
              apiKeyService.generateNew(
  ...
  public ResponseEntity<ApiKeyResponse> getApiKeySecretWithTotp(
    ...
    boolean totpValid = apiKeyService.verifyTotpForSecretViewing(userId, totpCode, purposeId);
  ...
  public ResponseEntity<TotpSetupResponse> setupSecretViewingTotp(
      @PathVariable String keyId, @CurrentUser User user) {

    String userId = user.getId();
    String email = user.getUsername();
    String purposeId =
        "secret-" + keyId + "-" + NanoIdUtils.randomNanoId() + "-" + System.currentTimeMillis();

    TotpSetupResponse response =
        totpUseCase.setupTotp(userId, email, TotpSecret.TotpPurpose.SECRET_KEY_VIEWING, purposeId);
  ...
  public ResponseEntity<List<ApiKeyResponse>> getAllApiKeys() {
    ...
    List<ApiKeyResponse> responses = apiKeys.stream().map(apiKeyMapper::toResponse).toList();
  ...
  public ResponseEntity<Boolean> validateApiKey(
      @RequestParam String keyId, @RequestParam String keySecret) {
