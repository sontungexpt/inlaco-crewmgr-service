...
public class CreateSupplyContractService
    implements CreateSupplyContractUseCase
{
    ...
    public Contract create(
        ...
        var crewRentalRequest = crewRentalRequestQueryUseCase.getRequest(
            requestId
        );
        ...
        if (contractFileAssetId != null) {
            log.debug(
                "Fetching contract file asset with ID: {}",
                contractFileAssetId
            );
            contract.setContractFile(
                uploadDispatcher.fetch(
                    AssetType.CONTRACT_FILE,
                    contractFileAssetId
                )
            );
        }
        ...
        if (shipImageAssetId != null) {
            log.debug(
                "Fetching ship image asset with ID: {}",
                shipImageAssetId
            );
            contract
                .getShipInfo()
        ...
        if (attachments != null && !attachments.isEmpty()) {
            log.debug(
                "Fetching {} attachment(s) for the contract",
                attachments.size()
            );
            contract.setAttachments(
                attachments
                    ...
                    .map(attachment -> {
                        log.debug(
                            "Fetching attachment with ID: {}",
                            attachment
                        );
                        return uploadDispatcher.fetch(
                            AssetType.CONTRACT_FILE,
                            attachment
                        );
                    })
        ...
        crewRentalRequestCommandUseCase.markSigning(
            crewRentalRequest,
            newContract.getId()
        );

        log.info(
            "Successfully created supply contract for crew rental request with id: {}",
            requestId
        );
