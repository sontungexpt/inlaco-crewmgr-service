...
public class CrewMobilizationRepositoryAdpater implements CrewMobilizationRepository {
  ...
  private static final String CREWS_FIELD = "crews";
  ...
  public Page<CrewMobilization> findAll(
    ...
    /*
     * Match mobilization fields first
     * => reduces dataset before lookup
     */
    Criteria mobilizationCriteria = buildMobilizationCriteria(criteria);

    if (mobilizationCriteria != null && !mobilizationCriteria.getCriteriaObject().isEmpty()) {

      operations.add(match(mobilizationCriteria));
    }

    /*
     * Only lookup assignments when filtering by accountId
     * => avoids unnecessary join
     */
    if (hasText(criteria != null ? criteria.getAccountId() : null)) {

      operations.add(
          lookup(
              mongoTemplate.getCollectionName(CrewMobilizationAssignmentEntity.class),
              "_id",
              "mobilizationId",
              CREWS_FIELD));

      operations.add(
          match(Criteria.where("crews.accountId").is(new ObjectId(criteria.getAccountId()))));
    }
    ...
    CrewMobilizationScheduleFacetResult result =
        mongoTemplate
            .aggregate(
                aggregation,
                CrewMobilizationEntity.class,
    ...
    return result.toPage(pageable).map(mapper::toCrewMobilization);
  }

  private Criteria buildMobilizationCriteria(CrewMobilizationSearchCriteria criteria) {
     ...
     * CLIENT ID
     *
     * partnerAccountId is String in entity
     * => DO NOT convert to ObjectId
     */
    if (hasText(criteria.getClientId())) {

      andCriteria.add(Criteria.where("partnerAccountId").is(criteria.getClientId()));
    }

    /*
     * SHIP IMO
     */
    if (hasText(criteria.getShipIMO())) {
...
// package
// com.inlaco.crewmgrservice.feature.crewmobilization.infrastructure.persistence.mongodb.adapter;
//
// import static org.springframework.data.mongodb.core.aggregation.Aggregation.*;
//
// import
// com.inlaco.crewmgrservice.feature.crewmobilization.application.model.CrewMobilizationSearchCriteria;
// import
// com.inlaco.crewmgrservice.feature.crewmobilization.application.port.out.CrewMobilizationRepository;
// import com.inlaco.crewmgrservice.feature.crewmobilization.domain.model.CrewMobilization;
// import
// com.inlaco.crewmgrservice.feature.crewmobilization.infrastructure.persistence.mongodb.entity.CrewMobilizationEntity;
// import
// com.inlaco.crewmgrservice.feature.crewmobilization.infrastructure.persistence.mongodb.mapper.CrewMobilizationEntityMapper;
// import
// com.inlaco.crewmgrservice.feature.crewmobilization.infrastructure.persistence.mongodb.repository.CrewMobilizationMongoRepository;
// import com.inlaco.crewmgrservice.infrastructure.persistence.mongodb.aggregation.FacetResult;
// import java.util.ArrayList;
// import java.util.List;
// import java.util.Optional;
...
//
//      andCriteria.add(Criteria.where("crews.accountId").is(new
// ObjectId(criteria.getAccountId())));
//    }
//    if (criteria.getClientId() != null && !criteria.getClientId().isBlank()) {
//      andCriteria.add(Criteria.where("partnerAccountId").is(new
// ObjectId(criteria.getClientId())));
//    }
//
